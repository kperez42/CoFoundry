...
enum CoFoundryError: LocalizedError, Identifiable {
    ...
    var errorDescription: String? {
        ...
        case .ageRestriction:
            return "You must be 18 or older to use CoFoundry."
        ...
        // Premium
        case .premiumRequired:
            return "This feature requires CoFoundry Premium."
        case .subscriptionExpired:
    ...
    static func from(_ error: Error) -> CoFoundryError {
        if let coFoundryError = error as? CoFoundryError {
            return coFoundryError
        }
...
struct ErrorAlert: ViewModifier {
    @Binding var error: CoFoundryError?
    ...
    private func errorMessage(for error: CoFoundryError) -> String {
...
extension View {
    func errorAlert(_ error: Binding<CoFoundryError?>) -> some View {
...
struct ErrorView: View {
    ...
    init(error: CoFoundryError, retryAction: (() -> Void)? = nil) {
        self.error = error
