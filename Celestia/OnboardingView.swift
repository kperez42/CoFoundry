...
//  CoFoundry
//
//  Professional Onboarding for Co-Founder Matching
//
...
struct OnboardingView: View {
    ...
    // Step 4: Co-Founder Preferences
    @State private var lookingFor = "Any Co-Founder"
    @State private var selectedSkills: [String] = []
    @State private var selectedIndustries: [String] = []
    @State private var selectedLanguages: [String] = []

    // Step 5: Startup Details
    @State private var startupStage: String = "Idea Stage"
    @State private var selectedRoleTypes: [String] = []
    @State private var timeCommitment: String = "Full-time"
    @State private var equityExpectation: String = "Negotiable"

    // Step 6: Experience & Background
    @State private var yearsExperience: Int = 0
    @State private var educationLevel: String = ""
    @State private var fieldOfStudy: String = ""
    @State private var previousStartups: Int = 0

    // Step 7: Funding & Investment
    @State private var currentlyFunded: Bool = false
    @State private var fundingExperience: String = ""
    @State private var investmentCapacity: String = ""

    // Step 8: Location Preferences
    @State private var locationPreference: String = "Remote"
    ...
    // Co-Founder Type Options
    let lookingForOptions = ["Technical Co-Founder", "Business Co-Founder", "Product Co-Founder", "Design Co-Founder", "Any Co-Founder"]

    // Startup Stage Options
    let startupStageOptions = ["Idea Stage", "MVP Building", "MVP Complete", "Early Traction", "Pre-Seed", "Seed Funded", "Series A+", "Scaling"]

    // Role Type Options
    let roleTypeOptions = ["Technical Co-Founder", "Business Co-Founder", "Product Co-Founder", "Design Co-Founder", "Marketing Co-Founder", "Operations Co-Founder"]

    // Time Commitment Options
    let timeCommitmentOptions = ["Full-time", "Part-time", "Nights & Weekends", "Flexible"]

    // Equity Expectation Options
    let equityExpectationOptions = ["Equal Split (50/50)", "Negotiable", "Based on Contribution", "Vesting Schedule", "Flexible"]

    // Location Preference Options
    let locationPreferenceOptions = ["Remote", "Local Only", "Hybrid", "Open to Relocation"]

    // Education Options
    let educationOptions = ["", "High School", "Some College", "Associate's Degree", "Bachelor's Degree", "Master's Degree", "MBA", "PhD", "Bootcamp Graduate"]

    // Field of Study Options
    let fieldOfStudyOptions = ["", "Computer Science", "Business/MBA", "Engineering", "Design", "Marketing", "Finance", "Economics", "Data Science", "Other"]

    // Funding Experience Options
    let fundingExperienceOptions = ["", "None", "Bootstrapped", "Angel Funded", "Pre-Seed", "Seed", "Series A+", "Accelerator/Incubator"]

    // Investment Capacity Options
    let investmentCapacityOptions = ["", "None", "$1K - $10K", "$10K - $50K", "$50K - $100K", "$100K+"]
    ...
    let availableSkills = [
        ...
        "Growth Hacking", "SEO/SEM", "Finance", "Operations",
        // Design
        "UI/UX Design", "Product Design", "Brand Design", "User Research"
    ]
    ...
    let availableIndustries = [
        ...
        "Traveltech", "Cleantech", "Biotech", "Hardware", "IoT", "Social Impact"
    ]
    ...
    private var stepTitle: String {
        switch currentStep {
        case 0: return "Professional Info"
        case 1: return "Your Bio"
        case 2: return "Profile Photo"
        case 3: return "Co-Founder Type"
        case 4: return "Skills & Industries"
        case 5: return "Startup Details"
    ...
    private var stepSubtitle: String {
        ...
        case 1: return "Share your professional story"
        case 2: return "Add a professional headshot"
        case 3: return "What you're looking for"
        case 4: return "Your expertise & interests"
        case 5: return "Your startup journey"
        case 6: return "Background & funding"
        case 7: return "Location & links"
        default: return ""
        }
    }
        ...
        ScrollView(showsIndicators: false) {
            VStack(spacing: 30) {
                ...
                VStack(spacing: 8) {
                    ...
                    Text("We need a few details to create your professional profile")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                ...
                VStack(spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        TextField("e.g. Full-Stack Engineer, Product Manager", text: $professionalTitle)
                            .padding()
                            .background(Color.white)
                            .cornerRadius(12)
                            .overlay(
                            ...
                            .accessibilityHint("Enter your current or most recent professional title")
                            .accessibilityIdentifier("professional_title_field")
                    }
                }
            }
            .padding(20)
        ...
        ScrollView(showsIndicators: false) {
            VStack(spacing: 30) {
                ...
                VStack(spacing: 8) {
                    ...
                    Text("Share your professional background and what you're building")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                        .multilineTextAlignment(.center)
                }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        TextEditor(text: $bio)
                            ...
                            .overlay(alignment: .topLeading) {
                                if bio.isEmpty {
                                    Text("Share your professional background, startup experience, and what you're looking to build...")
        ...
        ScrollView(showsIndicators: false) {
            VStack(spacing: 24) {
                ...
                VStack(spacing: 8) {
                    ...
                    Text("A professional photo builds trust with potential co-founders")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                        .multilineTextAlignment(.center)
    ...
    private var step4View: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 30) {
                ...
                VStack(spacing: 8) {
                    Text("Co-Founder Preferences")
                        .font(.title)
                        .fontWeight(.bold)

                    Text("What type of co-founder are you looking for?")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                }
    ...
    private var step5View: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 30) {
                ...
                VStack(spacing: 8) {
                    Text("Skills & Industries")
                        .font(.title)
                        .fontWeight(.bold)

                    Text("Select your expertise and industries of interest")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                }
                        ...
                        ForEach(availableSkills, id: \.self) { skill in
                            Button {
                                ...
                                Text(skill)
                                    .font(.subheadline)
                                    .fontWeight(.medium)
                                    .foregroundColor(selectedSkills.contains(skill) ? .white : .purple)
                        ...
                        ForEach(availableIndustries, id: \.self) { industry in
                            Button {
                                ...
                                Text(industry)
                                    .font(.subheadline)
                                    .fontWeight(.medium)
                                    .foregroundColor(selectedIndustries.contains(industry) ? .white : .green)
                                    .padding(.horizontal, 14)
                                    .padding(.vertical, 8)
                                    .background(
        ...
        ScrollView(showsIndicators: false) {
            VStack(spacing: 24) {
                ...
                VStack(spacing: 8) {
                    ...
                    Text("These details help find your ideal co-founder")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                        .multilineTextAlignment(.center)
